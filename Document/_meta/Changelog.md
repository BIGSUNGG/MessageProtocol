...
## 2026-09-22 (1) (3.2.1 — 코드 주석 영어화, 라이브러리 소비자 접근성)

- **버전 3.2.0 → 3.2.1** (패밀리 범프, 저장소 관례) — 패키지에 실리는 XML doc·주석·메타데이터 텍스트 전반의 변경이므로 패치 범프. 결정 배경·내용: [ADR-0008](../05-Decisions/ADR-0008-English-Code-Comments.md)

- **규약 확정**: 모든 코드 주석(`//`·`///` XML doc), 테스트 진단 문자열, 한국어 테스트 메서드명을 영어로 통일. 결정 배경·범위·예외: [ADR-0008](../05-Decisions/ADR-0008-English-Code-Comments.md), [CONVENTIONS](../00-AI/CONVENTIONS.md) "코드 주석" 섹션 신설. 라이브러리 소비자가 IntelliSense·예제·테스트 리포트에서 읽는 텍스트의 국제 접근성 확보가 목적.
- **변경 범위**: `Source/`·`Test/`·`Sandbox/` 57개 파일 — 한국어 주석 약 1,550줄 전량 영어화(단순 직역이 아니라 WHY/소비자 관점 재작성), 한국어 테스트 메서드명 259개 영어 snake_case 개명, 진단 문자열(Check·Assert 메시지·failures.Enqueue) 약 60곳 영어화. `Legacy/`(읽기 전용 아카이브)·`obj/`·`bin/`·`Document/` 볼트(한국어 유지)는 제외.
- **예외 유지**: UTF-8/비ASCII 직렬화 검증용 페이로드 리터럴(`"한글·日本語·🌟"`, `'한'`, `"경계값"` 등 16줄)은 테스트 데이터이므로 한국어 유지하고 각 위치에 의도를 밝히는 영어 주석 부착.
- **코드 무변경**: 와이어 포맷·공개 API 시그니처·코드 로직·테스트 로직 불변(diff는 주석·문자열 리터럴·식별자명만). README 미갱신 — 공개 API·기능 변경이 없기 때문.
- **검증**: `dotnet build` 오류 0(경고 4종은 사전 존재 확인), 단위 테스트 332×2 TFM(net8.0/net9.0) 전부 통과, Sandbox 전 시나리오 통과, 한국어 주석·한국어 식별자 grep 0줄, 잔여 한국어는 페이로드 화이트리스트 16줄뿐.
