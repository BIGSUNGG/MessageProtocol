...
# 상용화 적합성 검토 (2026-09-08, v2.3.9 기준; 2026-09-09 개선 패스 반영; **2026-09-13 v3.0.0 재검증**; **2026-09-14 v3.1.0 재검증**; **2026-09-14 v3.2.0 §4 Unity 생성기 제약 해소**)

> "실제 유니티 서버 상용화 서비스의 라이브러리로 사용해도 문제 없는가"에 대한 검토 기록.
> 빌드·테스트·퍼저·벤치마크 실측 + 와이어 호환성 실증 프로브(`artifacts/wire-probe/`) 기반.

## 3.2.0 갱신 (2026-09-14 — §4 Unity 생성기 제약 해소)

3.2.0 은 생성기의 Roslyn 참조 기본값을 4.3.0(Unity 6.0 LTS 번들 버전)으로 전환한 빌드 정책 릴리스다(생성기 기능 코드·와이어·런타임 무변경 — RS1024 pragma 주석만 추가, 배경은 [Changelog 2026-09-14 (5)](../_meta/Changelog.md)). 이로써 §4 의 'Unity 에디터 내장 Roslyn 보다 새로우면 분석기 로드 실패' 제약이 기본 패키지 기준으로 해소됐다 — nuget.org 표준 패키지가 Unity 6.0 에서 그대로 로드된다(구 참조 + 상위 컴파일러 하위호환으로 최신 SDK 소비자도 동일 패키지). 실측: 4.3 컴파일 생성기로 332/332 × 2 TFM 통과·Sandbox 전체 PASS·기본 경로 경고 0·3.2.0 nupkg AssemblyRef 4.3.0.0 검증. 잔존: 실제 Unity 에디터 에서의 로드 확인(§4 권고 3)은 여전히 1회 필요 — 단 이제 '실패 대응 경로'가 아니라 확인 절차.
...
- 단, **소스 생성기는 Microsoft.CodeAnalysis.CSharp 4.14 기준** — 대상 Unity 에디터 내장 Roslyn 보다 새로우면 분석기 로드 실패 가능. Unity 측 사용 전 실제 에디터에서 생성 동작 확인. 실패 시 수동 구현 경로 또는 서버 측에서 생성한 코드를 디스크로 고정해 사용. → **3.2.0 에서 기본 패키지가 Roslyn 4.3.0 참조로 전환되어 해소(위 3.2.0 갱신)**.
