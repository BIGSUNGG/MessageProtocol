...
이 저장소의 AI(pi) 하네스 구성과 그 목적. 하네스를 변경하면 이 문서와 [[../_meta/Changelog|Changelog]]를 갱신한다.

## 핵심 원칙 (AGENTS.md에도 명시됨)

1. **문서 우선** — 기능 추가/수정/제거 시 `Document/` Vault에 상세히 문서화. 작업 시 필요한 문서를 먼저 읽음
2. **능동 질문** — 모호하면 반드시 질문. 추측으로 진행 금지
3. **OOP/SOLID 준수** — 모든 코드 구현 (상세: `Document/00-AI/CONVENTIONS.md`)
...
1. 세션/작업 시작: `Document/00-AI/CONTEXT.md` → `Document/00-AI/GLOSSARY.md` → 작업 영역 문서 순서로 읽음 (doc-sync 스킬)
2. 변경 발생: doc-sync 스킬의 표에 따라 아키텍처/레퍼런스 문서·[[../_meta/Changelog|Changelog]]·ADR 갱신
...
- 모든 하네스 변경은 ADR + [[../_meta/Changelog|Changelog]] 기록 (doc-sync)
