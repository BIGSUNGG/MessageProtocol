...
버전 3.1.0 (재작성 버전. 패키지 아이디는 v1과 동일).
...
- 패키지 Version은 `Source/Directory.Build.props`에서 중앙 관리 (현재 `3.1.0`).
...
- **Unity 6.0 LTS 재빌드 (2026-09-14)** — Unity 6.0(6000.0.x) 에디터가 번들한 Roslyn 은 4.3.0 이다. 기본 빌드처럼 Microsoft.CodeAnalysis 4.14 을 참조한 생성기 DLL 은 CS9057 로 조용히 스킵된다(컴파일은 되지만 생성 코드가 없어 이후 실패). `Source/Directory.Build.props` 의 `RoslynAnalyzerApiVersion`(기본 4.14.0) 프로퍼티로 생성기의 Roslyn 참조 버전을 제어한다 — 기본 빌드/CI/nuget.org 게시 동작은 불변. Unity 피드용 재빌드: `dotnet pack Source/MessageProtocol.CodeGenerator/MessageProtocol.CodeGenerator.csproj -c Release -p:RoslynAnalyzerApiVersion=4.3.0 -p:Version=3.1.0-unity` → 로컬 피드 `C:/Projects/DS/unity-nuget/MessageProtocol.CodeGenerator.3.1.0-unity.nupkg` 로 복사. 실증(2026-09-14): nupkg 내부 DLL netstandard2.0·AssemblyRef Microsoft.CodeAnalysis(CSharp) 4.3.0.0 확인(System.Reflection.Metadata 검증). 4.3.0 경로에서만 RS1024 경고 1건(구버전 번들 분석기 오탐 — 4.14 기본 경로는 무경고).
